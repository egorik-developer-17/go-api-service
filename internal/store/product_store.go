...
func (s *ProductStore) Create(name string) model.Product { // метод для создания нового продукта
...
func (s *ProductStore) UpdateName(id int, name string) (model.Product, error) { // метод для обновления имени продукта по ID
	s.mu.Lock()         // блокируем для записи
	defer s.mu.Unlock() // разблокируем после записи
